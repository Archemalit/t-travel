...
class MemberControllerTest {
    ...
    void removeMember_shouldDeleteSuccessfully() throws Exception {
        ...
        verify(memberService).removeMember(eq(1L), eq(2L), any(User.class));
    }

    @Test
    @DisplayName("DELETE /trips/{tripId}/members/{userId} — поездка не найдена — выбрасывает TripNotFoundException")
    void removeMember_whenTripNotFound_shouldThrowTripNotFoundException() throws Exception {
        doThrow(new TripNotFoundException(1L)).when(memberService).removeMember(eq(1L), eq(2L), any(User.class));
    ...
    @Test
    @DisplayName("DELETE /trips/{tripId}/members/{userId} — пользователь не создатель — выбрасывает ForbiddenAccessException")
    void removeMember_whenUserNotCreator_shouldThrowForbiddenAccessException() throws Exception {
        doThrow(new ForbiddenAccessException("Только создатель поездки может удалять участников"))
                .when(memberService).removeMember(eq(1L), eq(2L), any(User.class));
    ...
    @Test
    @DisplayName("DELETE /trips/{tripId}/members/{userId} — участник не найден — выбрасывает ParticipantNotFoundException")
    void removeMember_whenParticipantNotFound_shouldThrowParticipantNotFoundException() throws Exception {
        doThrow(new ParticipantNotFoundException(1L, 2L))
                .when(memberService).removeMember(eq(1L), eq(2L), any(User.class));
